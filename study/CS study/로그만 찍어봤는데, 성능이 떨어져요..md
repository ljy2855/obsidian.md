...
고려해볼 수 있는건 IO device 작업을 완료될까지 기다리는게 되는데, 이를 IO blocking이라고 해요.
...
stdout(파일) 출력시에도 이정도 차이가 발생하게 돼요. 실제로 network 기반 로깅(kafka, elasticsearch) 시에 response 기반 
